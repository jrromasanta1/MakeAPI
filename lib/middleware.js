  ...
  return {
    prefixAuth: function( req, res, next ) {
      ...
      /*var email = req.body.email,
          makeTags = req.body.tags,
          appTags = req.body.appTags,*/
          var makeOriginal = req.body;
          
           makeOriginal = typeof makeOriginal === "string" ? [makeOriginal] : makeOriginal;
      ...
      for (var i in blacklist_words) {
        ...
        if (makeOriginal.toLowerCase().indexOf(blacklist_words[i]) != -1 &&  makeOriginal.toLowerCase().indexOf(" "+blacklist_words[i]+" ") != -1 ) {
           foundword[foundblacklistedcount] = blacklist_words[i];
           foundblacklistedcount++;
        }
      }
      //**********************************
      if (foundblacklistedcount != 0) {
